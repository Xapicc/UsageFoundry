...
**Thirty-three rows.** Thirty-four gap identifiers across the six axis files,
with
...
unbounded one and sits twenty-seventh, third from last among the open rows,
...
row 3 sits below row 2 despite the widest radius here, and why rows 27 and 28
are near the bottom of the open rows despite carrying the two unbounded ones.
Row 29 is below both and is the clearest case the rule has: it contradicts a
documented invariant and costs nothing, because there the code is safer than the
sentence.

**A closed row costs nothing to leave**, so rows 30 to 33 sit at the bottom. They
...
security rows and all eight operations rows answer *no*, and no number was
re-read on any of the three later passes** (no GitHub issue was opened, closed,
commented on or fetched), so every one of them is assumed unchanged since
`175ba57`. The one operations row that points anywhere points inside this
...
**Nine of thirty-three rows are one sentence: the app cannot find what it has
...
the documentation has an opinion, the code holds it: thirty-one of thirty-three
...
exactly that reason. Four more are a working mechanism reachable only up to a
number nobody chose: G1's three caps, G2's thirty. And
