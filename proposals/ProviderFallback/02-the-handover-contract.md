...
| 2 | assistant text → `finalText`, kind `assistant` | `item.completed` with `agent_message { text }` | **satisfied**, with a caveat: Claude's `finalText` is last-write-wins over turns; Codex's `-o <FILE>` is a purpose-built "last message" and is the more honest source. **Measured since, against 0.153.4:** the file is written on a turn that completes and **not** on `turn.failed`, so it is only the more honest source for a reader that deletes it before each spawn — left in place it hands the previous cycle's last word to a cycle that failed to produce one, and the last word is what ends the run. `orchestrator.ts`' `clearLastMessageFile` is that deletion. |
