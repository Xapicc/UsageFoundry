...
db.ts           SQLite: every table migrate() creates, and there are 37 —
...
**Every long-lived child also outranks the server as an OOM victim, and that is a second thing `privsep.ts` decides once.** Under a cgroup OOM the kernel scores each process at its share of `mem_limit` in thousandths and adds its `oom_score_adj`; Docker sets neither that nor `memory.oom.group`, so every process in the container starts at 0 and the kill goes to whichever is biggest — which is the wrong answer whenever that is the server, because killing a work cycle costs a cycle and killing the server costs every run in flight. `deprioritiseChildForOom` writes `CHILD_OOM_SCORE_ADJ` to `/proc/<pid>/oom_score_adj` one line after each of the four spawns above and after the prune that executes a cut, and to nothing else: `git.ts`'s children, `claudeAuth.ts`'s and winnow's three reading calls are megabytes and seconds, and preferring one of those is a kill that frees nothing. The offset is the same on every child that carries it, so which child dies is still decided by which one is biggest; what it is derived from is the bound `deployment.test.ts` puts on the server's heap ceiling, which is why that file asserts the two against each other rather than asserting a number. The write is best-effort and warned about once per process — a spawn that threw over an OOM preference would be an outage of its own.
