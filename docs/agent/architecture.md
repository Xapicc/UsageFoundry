...
db.ts           SQLite: every table migrate() creates, and there are 34 —
                ...
                fork_attempts, resume_probes, plan_observations, agents,
                settings, chat_sessions, chat_messages, chat_proposals,
                chat_questions, chat_turn_spend, workflows, workflow_instances,
                workflow_instance_runs, workflow_instance_blocks,
                workflow_schedules, context_samples, context_compositions,
                context_composition_children, prune_decisions, prune_receipts,
                dreaming_nights, dreaming_notes, merge_queue, ops_events,
                request_log, otlp_requests, webhook_deliveries, auth_sessions,
                login_attempts. The list is a completeness claim, so check
                it against
