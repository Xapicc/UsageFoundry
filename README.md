...
| A tool the agents were told to have is not there | `tools.notOk` | `> 0` — a declared `UF_PY_TOOLS` or `UF_GH_EXTENSIONS` entry whose install did not complete, or whose command no longer resolves on the PATH a run gets. Counts the two danger readings only, not a tool that is shadowed or returning errors; the names are at `/api/tools`, behind the master token rather than this one. Recomputed per request rather than per boot, so it falls again the moment the tool is there — threshold it on `> 0` and never on "changed since the last boot" |
