...
function handleStreamLine(
  ...
  // The provider's own reading of the account's two windows. Nothing consumes it
  // since the card that showed it was removed, and the provider emits it on most
  // turns, so it is named here only to keep the fallback below from filing a
  // routine event as a vocabulary this app has lost.
