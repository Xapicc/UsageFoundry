...
describe("handleStreamLine on an unrecognised event type", () => {
  ...
  // `rate_limit_event` has no consumer since the provider-reported card was
  // removed, but the provider still emits it on most turns — so the branch that
  // ignores it is what this pins. Delete that branch and every run files a
  // routine event as a vocabulary the app has lost, silently and repeatedly.
  it("does not file the rate-limit event as a vocabulary it has lost", () => {
    ...
    // them, so the shape is the one the provider actually sends.
